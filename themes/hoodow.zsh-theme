...
prompt_hoodow_setup() {
  ...
  typeset -Ag hoodow_prompt_colors
  hoodow_prompt_colors=(${(kv)pc})
  ...
  RPROMPT="\$(prompt_hoodow_scm_status)"
  RPROMPT+="%(?.. $pc[rc]exited %1v$pc[reset])"

  export PROMPT RPROMPT
  precmd_functions+='prompt_hoodow_precmd'
}

prompt_hoodow_precmd() {
  local ex=$?
  psvar=()
...
prompt_hoodow_scm_status() {
  zgit_isgit || return
  local -A pc
  pc=(${(kv)hoodow_prompt_colors})
